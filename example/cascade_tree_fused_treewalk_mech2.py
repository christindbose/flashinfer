...
parser.add_argument('--merge_every_n_levels', type=int, default=1,
                    help='Merge every N levels (default: 1, merges all levels). '
                         'Set to 2 to merge every 2nd level (e.g., levels 1,3 for a 4-level tree)')
parser.add_argument('--kvsplit_mode', action='store_true', default=False,
                    help='Enable KV split mode (default: False)')
parser.add_argument('--mech2_mode', action='store_true', default=False,
                    help='Enable mech2 mode (default: False)')
...
print(f"Merge every N levels: {args.merge_every_n_levels}")
print(f"KV split mode:      {args.kvsplit_mode}")
print(f"Mech2 mode:         {args.mech2_mode}")
...
for level in range(num_levels):
    ...
    indent = "  " * level
    will_merge = (level > 0 and (level - 1) % args.merge_every_n_levels == 0) or level == 0
    merge_indicator = " [BASE]" if level == 0 else (" [MERGE]" if will_merge else " [SKIP]")
    print(f"{indent}Level {level}: {nodes[level]} node(s), {contexts[level]} tokens, {seqs_per_node} seqs/node{merge_indicator}")
...
wrapper.plan(
    ...
    kvsplit_mode=args.kvsplit_mode,
    mech2_mode=args.mech2_mode,
)
...
merged_out = wrapper.run(q, kv_cache, tree_nodes=nodes, merge_every_n_levels=args.merge_every_n_levels)
